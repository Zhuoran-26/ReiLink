...
class ConversationStore:
    ...
    def append(
        ...
        assistant_message_type: str = "reply",
    ...
    def append_proactive(
        ...
        self.append(
            ...
            timestamp=timestamp,
            assistant_reply_segments=[assistant_reply],
            assistant_message_type="proactive",
            trigger_type=trigger_type,
    ...
    def recent_context(self, session_id: str, limit: int = 1) -> list[dict[str, str]]:
        entries = [entry for entry in self.read_session(session_id) if entry.assistant_message_type != "proactive"][-limit:]
        return [
            {
    ...
    def recent_assistant_replies(self, session_id: str, limit: int = 5) -> list[str]:
        entries = [entry for entry in self.read_session(session_id) if entry.assistant_message_type != "proactive"]
        return [entry.assistant_reply for entry in entries[-limit:]]

    def recent_user_messages(self, session_id: str, limit: int = 8) -> list[str]:
        entries = [
            entry
            for entry in self.read_session(session_id)
            if entry.assistant_message_type != "proactive" and entry.user_message
        ]
        return [entry.user_message for entry in entries[-limit:]]
