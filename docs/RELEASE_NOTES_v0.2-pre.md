...
- Bundled runtime resources：persona、persona style 和 game registry 等只读 runtime resources 会随 packaged app 分发，memory / session / settings / logs 仍写入用户数据目录。
...
- Bundled runtime resources: read-only runtime resources such as persona data, persona style, and the game registry are distributed with the packaged app, while memory, session, settings, and logs remain in the user data directory.
