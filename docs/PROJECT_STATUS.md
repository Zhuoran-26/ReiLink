...
- Semantic Shadow 真实 provider 请求已收敛为紧凑 JSON-only 输出：prompt 明确禁止 Markdown / 解释 / code fence，并优先使用 provider JSON object mode；若 JSON mode 返回不可解析内容，后台 Shadow 会做一次不带 `response_format` 的兼容 retry。解析器只做安全轻量恢复，可接受严格 JSON、Markdown code fence、前后夹杂简短说明的首个 JSON object，以及数组中的首个 object。
- Semantic Shadow final event 会附带安全诊断字段：`response_format_used`、`compat_retry_used`、`json_recovery_stage`、`finish_reason`、`content_length_bucket`、`first_char_type`。这些字段只用于 Debug / Event Stream 定位 provider 兼容性问题，不包含 raw provider response、raw prompt、完整用户输入、密钥或路径。
...
- Real-provider Semantic Shadow requests now use a compact JSON-only contract: the prompt explicitly forbids Markdown, explanations, and code fences, and prefers provider JSON object mode. If JSON mode returns unparseable content, the background Shadow task performs one compatibility retry without `response_format`. The parser only performs safe lightweight recovery for strict JSON, Markdown code fences, brief prose around the first JSON object, and arrays containing a first object.
- Semantic Shadow final events include safe diagnostics: `response_format_used`, `compat_retry_used`, `json_recovery_stage`, `finish_reason`, `content_length_bucket`, and `first_char_type`. These fields are Debug / Event Stream observability only and do not include raw provider responses, raw prompts, full user input, secrets, or paths.
