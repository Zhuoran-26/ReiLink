...
`reilink-mvp-v0.1.1` 已经作为公开展示版本发布，用于 GitHub / portfolio / interview 展示。`reilink-v0.2-pre` 已作为预发布版本公开，当前 `dev/codex-reilink` 已进一步补齐 standalone runtime / productization foundation，并阶段性完成 Voice Interaction MVP：可选系统 TTS、本地 ASR 主聊天输入、transcript-first 用户确认发送、Local ASR Settings 持久化和 release regression freeze。当前已完成 Overlay v1 Foundation，接入 Voice Interaction v2.1、Voice Profile v1、TTS Strategy Spike v0 与 TTS Provider Registry / Capability Surface v0，并完成 LLM-primary Guarded Extraction v1 pilot 与 Extraction Eval Runner v0 / v1.0.3：前景 LLM semantic reader、严格候选 schema、formal game state 与 candidate understanding 分层、deterministic guard、text / voice_confirmed / voice_direct 来源 metadata、rule fallback、安全 trace、固定 mock eval 回归和可选 live provider drift check。Hermes-style Memory Architecture v0 已作为文档与 QA 场景基线加入，Candidate Memory v1 已实现最小运行时，Memory Retrieval v1 已接入 accepted / active long-term memory 的 bounded prompt assembly，Persona-Memory Eval v0.1 已加入 mock-first 回归入口与 live warning tiers，Session Archive v1 Architecture / Runtime / Search v1 已加入 safe-summary local archive、最近会话 UI 与安全关键词 / 过滤搜索，Archive-to-Memory Candidate Bridge v0 已加入显式扫描、deterministic safe-summary detector、memory guard / pending queue 复用和用户确认流；Context & Memory release hardening checklist 与 v0.2-pre.4 release notes 草稿已加入。向量库、semantic archive search、prompt archive retrieval、archive search auto-candidate、archive scan audit counts、advanced archive UI、外部 memory provider、真实外部 / 本地 TTS provider、Overlay auto-show 和 Live2D / Vision 仍未实现。
...
- Voice Interaction v2.1 + Voice Profile v1 + TTS Strategy v0 + TTS Provider Registry v0：直接语音对话 loop 的状态机、confirm-send 默认、直接对话显式 opt-in、TTS 打断、录音 / 播放互斥、Home / Chat 紧凑状态、Voice workspace Conversation 状态面板、规则化 full / brief / silent 播报策略，以及 renderer 端 `system_speech_synthesis` strategy / fallback 与只读 provider 能力展示；hands-free、角色 TTS / 角色音色、真实外部 / 本地 TTS provider 和 Overlay voice state 仍未实现。
...
- Voice Input v1 已完成 push-to-talk fallback：`confirm_send` 默认模式下 Web Speech transcript 只填入输入框，不自动发送；未确认 transcript 不进入 memory、prompt、knowledge retrieval 或 game context。v2.1 直接对话模式开启后，Web Speech / Local ASR transcript 会在用户主动录音结束后进入正常 chat flow。
