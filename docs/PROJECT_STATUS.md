...
- Persona-Memory Regression Eval v0：新增 mock-first eval runner、20+ 条机器可读场景和粗粒度安全 / 风格指标，验证 retrieved memory 对回复的影响自然、低优先级、persona-safe，不机械复述“我记得你”，不使用 pending / undone / rejected memory，不暴露 prompt / guard / memory 机制；可选 live provider drift check 不进入强制 CI。
...
- Persona-Memory Regression Eval v0: adds a mock-first eval runner, 20+ machine-readable scenarios, and coarse safety / style metrics to verify retrieved memory influences replies naturally, remains low-priority and persona-safe, does not mechanically repeat "I remember", does not use pending / undone / rejected memory, and does not expose prompt / guard / memory mechanics. Optional live-provider drift checks remain manual and outside required CI.
