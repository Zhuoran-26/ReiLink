...
- Session Archive v1 Runtime + Search v1：在 Architecture / QA baseline 上新增手动 `archive-current`、本地 `data/session/session_archives.json` safe-summary 持久化、list / read / delete / clear / search API、Memory workspace `最近会话` tab、关键词 / game / boss / event type / date 过滤搜索、renderer 安全事件和 20+ runtime / search QA scenarios。Archive 只保存并搜索 Session Timeline safe summaries，不保存 raw chat / prompt / JSON / secret / full path / audio / transcript，不进入 PromptMemoryBlock / Memory Retrieval / Long-term Memory；Archive-to-Memory Candidate、向量库和 prompt archive retrieval 仍未实现。
