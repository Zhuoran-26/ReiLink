...
- Semantic Shadow 真实 provider 请求已收敛为紧凑 JSON-only 输出：prompt 明确禁止 Markdown / 解释 / code fence，并优先使用 provider JSON object mode；解析器只做安全轻量恢复，可接受严格 JSON、Markdown code fence、前后夹杂简短说明的首个 JSON object，以及数组中的首个 object。
...
- Real-provider Semantic Shadow requests now use a compact JSON-only contract: the prompt explicitly forbids Markdown, explanations, and code fences, and prefers provider JSON object mode. The parser only performs safe lightweight recovery for strict JSON, Markdown code fences, brief prose around the first JSON object, and arrays containing a first object.
