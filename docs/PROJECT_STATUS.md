...
- Candidate Memory v1.1 已接入运行时：显式记忆经 Memory Candidate guard 后会自动写入可见 `long_term_memories`，聊天页显示轻量可撤销提示；稳定隐式玩法偏好、稳定互动偏好、剧透偏好等仍进入 pending candidate。Memory Candidate Check 保持 LLM-primary：规则只做 fast prefilter / grounding / deterministic fallback，偏好类输入不会因为没有显式“记住”而被规则层吞掉。否定记忆、单次 session event、assistant / proactive 来源、secret 和 persona drift 会被 guard 阻断；active accepted memory 可进入现有 prompt memory context，undone / inactive、pending、ignored、expired、rejected 不进入 prompt。配套场景位于 `docs/qa/candidate_memory_scenarios.json` 与 `docs/qa/memory_ux_v1_1_scenarios.json`。当前仍不实现 Memory Retrieval、Session Archive、向量数据库或外部 memory provider。
