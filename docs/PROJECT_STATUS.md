...
- 置信度设计保持可解释：高置信规则直接应用；被动死亡、near-clear、指代不明、中文游戏失败 slang 等“容易误判”的表达会降低调度置信度以触发 LLM 影子识别。provider 不可用、auth_failed、timeout、invalid JSON、provider error 或 no meaningful update 时也会记录安全 trace，而不是 silent no-op。
