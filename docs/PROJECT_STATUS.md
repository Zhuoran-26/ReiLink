...
- Session Archive v1 Runtime：在 Architecture / QA baseline 上新增手动 `archive-current`、本地 `data/session/session_archives.json` safe-summary 持久化、list / read / delete / clear API、Memory workspace `最近会话` tab、renderer 安全事件和 20+ runtime QA scenarios。Archive 只保存 Session Timeline safe summaries，不保存 raw chat / prompt / JSON / secret / full path / audio / transcript，不进入 PromptMemoryBlock / Memory Retrieval / Long-term Memory；Archive Search、Archive-to-Memory Candidate、向量库和 prompt archive retrieval 仍未实现。
...
- Session Archive v1 Runtime 已接入：`docs/session_archive_v1_architecture.md` 继续定义 safe session history / search / memory candidate bridge 的产品定位、层级边界、内容边界、默认策略、数据模型草案、Archive -> Memory Candidate 流程、Search 设计、UI 规划和 privacy / safety 规则；架构场景位于 `docs/qa/session_archive_scenarios.json`，runtime 场景位于 `docs/qa/session_archive_runtime_scenarios.json`。当前 runtime 只实现手动 safe-summary archive、本地持久化、最近会话 UI、list / read / delete / clear、重复归档幂等和空 timeline skipped；不实现 search runtime、Archive-to-Memory Candidate runtime、prompt archive retrieval 或 vector search。
...
- Session Archive Search v1 / Archive-to-Memory Candidate v1 / archive runtime controls expansion。
...
- Session Archive v1 Runtime: adds manual `archive-current`, local `data/session/session_archives.json` safe-summary persistence, list / read / delete / clear APIs, the Memory workspace Recent Sessions tab, safe renderer events, and 20+ runtime QA scenarios on top of the Architecture / QA baseline. Archive stores only Session Timeline safe summaries, never raw chat / prompt / JSON / secrets / full paths / audio / transcripts, and it does not enter PromptMemoryBlock, Memory Retrieval, or Long-term Memory. Archive Search, Archive-to-Memory Candidate runtime, vector search, and prompt archive retrieval remain unimplemented.
...
- Session Archive v1 Runtime is wired in. `docs/session_archive_v1_architecture.md` still defines safe session history / search / memory candidate bridge positioning, layer boundaries, content boundaries, default policy, data-model drafts, Archive -> Memory Candidate flow, Search design, UI planning, and privacy / safety rules. Architecture scenarios live in `docs/qa/session_archive_scenarios.json`; runtime scenarios live in `docs/qa/session_archive_runtime_scenarios.json`. Current runtime scope is manual safe-summary archive, local persistence, Recent Sessions UI, list / read / delete / clear, repeated archive idempotency, and empty timeline skipped responses. Search runtime, Archive-to-Memory Candidate runtime, prompt archive retrieval, and vector search remain unimplemented.
