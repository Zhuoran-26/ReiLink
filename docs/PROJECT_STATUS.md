...
- Semantic Extraction v1.0.1 pilot now has a foreground LLM-primary path: the LLM only proposes candidates, and schema validation plus deterministic guard decides `apply`, `ask_clarification`, `candidate_only`, `no_op`, or `fallback_to_rule`. Rules act as grounding / safe fallback, not the primary reader. Only deterministic events marked `guard_source=llm_primary` can write game context; memory, proactive, and persona are not written by this path.
...
- Semantic Extraction Debug now exposes a safe LLM Primary Extraction trace: `source` (`llm_primary` / rule / none), `input_source`, `confidence`, `llm_primary_status`, `llm_provider_status`, `llm_schema_valid`, `fallback_reason`, `skip_reason`, `applied_updates`, `rule_grounding`, guard decision / reason / summary, plus legacy `llm_shadow_status`, `llm_shadow_summary`, and `llm_shadow_diff`. Debug / Event Stream summaries do not expose full user messages, raw prompts, raw JSON, paths, or transcripts.
