...
- LLM-first 回复生成。
- 游戏上下文、记忆和知识层提供辅助信息。
- 当前 sample companion 是 ReiLink 原创 Rei-like persona，不使用官方 IP；v1 已开始使用结构化 Persona Pack 组织角色边界。
...
- Prompt Preview。
- Rei Persona Pack v1：结构化原创 persona 文件、safe loader、prompt assembly 接入和 Debug Prompt Preview 安全摘要。
- Debug Dashboard。
...
- Rei Persona Pack v1 已建立在 `personas/rei/`，包含 persona、voice、boundaries、game companion policy、memory policy、proactive policy、examples、anti examples、references 和 version metadata。
...
- 主聊天 prompt 现在在 base system safety / app identity 后注入 structured Rei Persona Pack，再继续使用游戏上下文、已确认长期记忆、知识检索和当前用户输入；pack 不能覆盖安全、隐私、memory confirmation、knowledge grounding 或 proactive gating。
- Persona Pack prompt 注入有固定长度预算：persona、voice、boundaries 和 policy sections 为主，examples / anti_examples 只注入少量精选；内容过长时安全截断并在 Debug summary 显示 `persona_section_truncated` 和 truncated_sections。
...
- Persona Pack v1 不做用户自定义角色、Persona 自动学习、Voice Profile、TTS 音色、Live2D 或 LLM-primary 状态写入；memory、proactive 和 Semantic Shadow 的状态写入边界保持不变。
...
- The current sample companion is an original ReiLink Rei-like persona and does not use official IP; v1 now uses a structured Persona Pack to organize character boundaries.
...
- Event Bus / Event Stream.
- Prompt Preview.
- Rei Persona Pack v1: structured original persona files, safe loader, prompt assembly integration, and safe Debug Prompt Preview summary.
- Debug Dashboard.
- QA / Regression scenarios.
- UI polish.
...
- Rei Persona Pack v1 now lives in `personas/rei/` with persona, voice, boundaries, game companion policy, memory policy, proactive policy, examples, anti examples, references, and version metadata.
...
- Main chat prompt assembly now injects the structured Rei Persona Pack after base system safety / app identity, then continues with game context, confirmed long-term memory, knowledge retrieval, and the current user input. The pack cannot override safety, privacy, memory confirmation, knowledge grounding, proactive gating, or the LLM Shadow candidate-only boundary.
- Persona Pack prompt injection has a fixed length budget: persona, voice, boundaries, and policy sections are primary, while examples / anti_examples inject only a small stable selection. Overlong content is safely truncated and Debug summary reports `persona_section_truncated` and truncated_sections.
...
- Persona Pack v1 does not implement custom characters, persona self-learning, Voice Profile, TTS voice models, Live2D, or LLM-primary state writes. Memory, proactive, and Semantic Shadow write boundaries remain unchanged.
