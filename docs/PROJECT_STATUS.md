...
`reilink-mvp-v0.1.1` 已经作为公开展示版本发布，用于 GitHub / portfolio / interview 展示。`reilink-v0.2-pre` 已作为预发布版本公开，当前 `dev/codex-reilink` 已进一步补齐 standalone runtime / productization foundation，并阶段性完成 Voice Interaction MVP：可选系统 TTS、本地 ASR 主聊天输入、transcript-first 用户确认发送、Local ASR Settings 持久化和 release regression freeze。当前已完成 Overlay v1 Foundation，接入 Voice Interaction v2.1、Voice Profile v1 与 TTS Strategy Spike v0，并完成 LLM-primary Guarded Extraction v1 pilot 与 Extraction Eval Runner v0 / v1.0.3：前景 LLM semantic reader、严格候选 schema、formal game state 与 candidate understanding 分层、deterministic guard、text / voice_confirmed / voice_direct 来源 metadata、rule fallback、安全 trace、固定 mock eval 回归和可选 live provider drift check。Hermes-style Memory Architecture v0 已作为文档与 QA 场景基线加入，Candidate Memory v1 已实现最小运行时，Memory Retrieval v1 已接入 accepted / active long-term memory 的 bounded prompt assembly，Persona-Memory Eval v0.1 已加入 mock-first 回归入口与 live warning tiers，Session Archive v1 Architecture / Runtime / Search v1 已加入 safe-summary local archive、最近会话 UI 与安全关键词 / 过滤搜索，Archive-to-Memory Candidate Bridge v0 已加入显式扫描、deterministic safe-summary detector、memory guard / pending queue 复用和用户确认流；Context & Memory release hardening checklist 与 v0.2-pre.4 release notes 草稿已加入。向量库、semantic archive search、prompt archive retrieval、archive search auto-candidate、archive scan audit counts、advanced archive UI、外部 memory provider、外部 / 本地 TTS provider、Overlay auto-show 和 Live2D / Vision 仍未实现。
...
- Voice Output：`语音输出 / Voice Output`、Test Voice、rate / volume、中文语音优先、TTS Strategy v0 `system_speech_synthesis` fallback 和 Event Stream 安全摘要。
...
- Voice Interaction v2.1 + Voice Profile v1 + TTS Strategy v0：直接语音对话 loop 的状态机、confirm-send 默认、直接对话显式 opt-in、TTS 打断、录音 / 播放互斥、Home / Chat 紧凑状态、Voice workspace Conversation 状态面板、规则化 full / brief / silent 播报策略，以及 renderer 端 `system_speech_synthesis` strategy / fallback；hands-free、角色 TTS / 角色音色、外部 / 本地 TTS provider 和 Overlay voice state 仍未实现。
...
- TTS Strategy Spike v0 已接入 renderer：当前唯一 strategy 是 `system_speech_synthesis`，封装本机系统 `speechSynthesis` 的 speak / stop / availability fallback；Voice workspace 显示当前 strategy，Direct Conversation 会把 `brief` / `full` / `silent` profile 传入 TTS lifecycle，Event Stream 只记录 strategy id、source、profile、字数、状态和错误摘要，不保存完整 assistant reply 或 spoken text。
...
- Voice 的产品位置升级为一级模块：当前是 Local ASR transcript-first 默认、Voice Output、Voice Interaction v2.1 direct conversation opt-in、Voice Profile v1 行为策略和 TTS Strategy v0 system fallback；`docs/voice_interaction_v2_spec.md` 覆盖并接入 `idle`、`listening`、`transcribing`、`ready_to_send`、`assistant_thinking`、`speaking`、`interrupted`、`error` 等状态。hands-free、角色 TTS / 角色音色、外部 / 本地 TTS provider 和 Overlay voice state 仍未实现。
...
- Elden Ring sample knowledge pack.
- Hollow Knight sample knowledge pack.
- Voice Output: Voice Output settings, Test Voice, rate / volume, Chinese voice preference, TTS Strategy v0 `system_speech_synthesis` fallback, and safe Event Stream summaries.
...
- Voice Interaction v2.1 + Voice Profile v1 + TTS Strategy v0: direct spoken conversation loop state machine, confirm-send default, explicit opt-in Direct Conversation Mode, short / partial transcript auto-send guard, TTS interruption, recording / speaking mutual exclusion, compact Home / Chat state, Voice workspace Conversation state panel, rule-based full / brief / silent spoken reply policy, and renderer-side `system_speech_synthesis` strategy / fallback; hands-free, character TTS / character voice, external / local TTS providers, and Overlay voice state remain unimplemented.
...
- TTS Strategy Spike v0 is wired into the renderer. The only current strategy is `system_speech_synthesis`, which wraps local system `speechSynthesis` speak / stop / availability fallback. The Voice workspace shows the current strategy, Direct Conversation passes `brief` / `full` / `silent` profile metadata into TTS lifecycle events, and Event Stream records only strategy id, source, profile, character counts, status, and error summaries without storing full assistant replies or spoken text.
...
- Voice Interaction v2.1 + Voice Profile v1 are wired into the renderer. `docs/voice_interaction_v2_spec.md` defines the direct conversation loop, `idle` / `listening` / `transcribing` / `ready_to_send` / `assistant_thinking` / `speaking` / `interrupted` / `error` states, confirm-send default, explicit `direct_conversation` opt-in, short / partial transcript auto-send guard, TTS / recording mutual exclusion, Direct Conversation brief auto-speak when Voice Output is enabled, safe error handling, and UI placement. `docs/voice_profile_v1.md` defines full / brief / silent policy, never-spoken categories, and TTS Strategy v0 event boundaries. Machine-readable scenarios live in `docs/qa/voice_interaction_v2_scenarios.json` and `docs/qa/voice_profile_scenarios.json`. The current implementation scope is not hands-free, character TTS / character voice, external / local TTS providers, or Overlay voice state.
