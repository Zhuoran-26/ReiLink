...
- Persona-Memory Regression Eval v0：新增 mock-first eval runner 和机器可读场景，验证 retrieved memory 对回复的影响自然、低优先级、persona-safe，不机械复述“我记得你”，不使用 pending / undone / rejected memory，不暴露 prompt / guard / memory 机制；可选 live provider drift check 不进入强制 CI。
...
- Persona-Memory Regression Eval v0 已加入：`docs/qa/persona_memory_regression_scenarios.json` 覆盖 accepted gameplay / interaction / spoiler / accessibility memory 的自然影响、当前输入优先、pending / rejected / undone 不注入、persona drift 阻断、safe summary only 和 voice_direct brief 场景。`python scripts/run_persona_memory_eval.py --provider mock` 是固定 mock 回归入口；`--provider live --allow-failures` 仅用于人工观察真实 provider 漂移。
...
- Persona-Memory Regression Eval v0: adds a mock-first eval runner and machine-readable scenarios to verify retrieved memory influences replies naturally, remains low-priority and persona-safe, does not mechanically repeat "I remember", does not use pending / undone / rejected memory, and does not expose prompt / guard / memory mechanics. Optional live-provider drift checks remain manual and outside required CI.
