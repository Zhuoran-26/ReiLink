...
- Session Archive v1 Runtime + Search v1 已接入：`docs/session_archive_v1_architecture.md` 继续定义 safe session history / search / memory candidate bridge 的产品定位、层级边界、内容边界、默认策略、数据模型草案、Archive -> Memory Candidate 流程、Search 设计、UI 规划和 privacy / safety 规则；架构场景位于 `docs/qa/session_archive_scenarios.json`，runtime 场景位于 `docs/qa/session_archive_runtime_scenarios.json`，search 场景位于 `docs/qa/session_archive_search_scenarios.json`。当前 runtime 实现手动 safe-summary archive、本地持久化、最近会话 UI、list / read / delete / clear / search、重复归档幂等和空 timeline skipped；不实现 Archive-to-Memory Candidate runtime、prompt archive retrieval 或 vector search。
...
- Archive-to-Memory Candidate v1 / archive runtime controls expansion / future semantic archive search。
...
- Session Archive v1 Runtime + Search v1: adds manual `archive-current`, local `data/session/session_archives.json` safe-summary persistence, list / read / delete / clear / search APIs, the Memory workspace Recent Sessions tab, keyword / game / boss / event type / date filters, safe renderer events, and 20+ runtime / search QA scenarios on top of the Architecture / QA baseline. Archive stores and searches only Session Timeline safe summaries, never raw chat / prompt / JSON / secrets / full paths / audio / transcripts, and it does not enter PromptMemoryBlock, Memory Retrieval, or Long-term Memory. Archive-to-Memory Candidate runtime, vector search, and prompt archive retrieval remain unimplemented.
...
- Session Archive v1 Runtime + Search v1 is wired in. `docs/session_archive_v1_architecture.md` still defines safe session history / search / memory candidate bridge positioning, layer boundaries, content boundaries, default policy, data-model drafts, Archive -> Memory Candidate flow, Search design, UI planning, and privacy / safety rules. Architecture scenarios live in `docs/qa/session_archive_scenarios.json`; runtime scenarios live in `docs/qa/session_archive_runtime_scenarios.json`; search scenarios live in `docs/qa/session_archive_search_scenarios.json`. Current runtime scope is manual safe-summary archive, local persistence, Recent Sessions UI, list / read / delete / clear / search, repeated archive idempotency, and empty timeline skipped responses. Archive-to-Memory Candidate runtime, prompt archive retrieval, and vector search remain unimplemented.
