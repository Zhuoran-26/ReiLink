...
- LLM-first 回复生成。
- 游戏上下文、记忆和知识层提供辅助信息。
- 当前 sample companion 是 ReiLink 原创 Rei-like persona，不使用官方 IP；v1.1.1 已开始使用中文优先的结构化 Persona Pack 校准冷静寡言陪伴风格。
...
- Prompt Preview。
- Rei Persona Pack v1.1.1：中文优先的结构化原创 persona 文件、冷静寡言风格校准、真人感与重复回归修正、safe loader、prompt assembly 接入和 Debug Prompt Preview 安全摘要。
- Debug Dashboard。
...
- Rei Persona Pack v1.1.1 已建立在 `personas/rei/`，包含 persona、voice、boundaries、game companion policy、memory policy、proactive policy、style calibration、response patterns、examples、anti examples、references 和 version metadata。
- v1.1.1 的 runtime-facing persona markdown 已改为中文优先，目标是更稳定地呈现冷静、寡言、低情绪、有距离感、少量但真实关心的游戏陪伴风格。
- v1.1.1 风格校准已进一步收敛到“表达通道窄”的原创冷感画像：不是无情或空白，而是反应压低、先观察事实、少解释感受；同时补充了非硬编码的重复回复防护，允许相近意思的自然变体，但禁止机械复读或滥用固定过渡词。v1.1.1 还新增 `docs/qa/persona_regression_cases.json`，把手测失败模式整理为可持续回归样例。
...
- Persona Pack v1.1.1 不做用户自定义角色、Persona 自动学习、Voice Profile、TTS 音色、Live2D 或 LLM-primary 状态写入；memory、proactive 和 Semantic Shadow 的状态写入边界保持不变。
...
- The current sample companion is an original ReiLink Rei-like persona and does not use official IP; v1.1.1 now uses a Chinese-first structured Persona Pack to calibrate a cold, quiet companion style.
...
- Event Bus / Event Stream.
- Prompt Preview.
- Rei Persona Pack v1.1.1: Chinese-first structured original persona files, cold quiet companion calibration, human-feel and repetition regression fixes, safe loader, prompt assembly integration, and safe Debug Prompt Preview summary.
- Debug Dashboard.
- QA / Regression scenarios.
- UI polish.
...
- Rei Persona Pack v1.1.1 now lives in `personas/rei/` with persona, voice, boundaries, game companion policy, memory policy, proactive policy, style calibration, response patterns, examples, anti examples, references, and version metadata.
- v1.1.1 runtime-facing persona markdown is Chinese-first and calibrates Rei toward calm, terse, low-emotion, slightly distant, lightly caring game companionship.
- v1.1.1 calibration now further narrows Rei toward an original narrow-expression cold persona: not emotionless or blank, but low-output, fact-first, and reluctant to explain feelings. It also adds non-hardcoded repetition control: similar meaning may use natural variation, while mechanical repeats and overused transition words are forbidden. v1.1.1 also adds `docs/qa/persona_regression_cases.json` to keep hand-test failure modes as sustainable regression cases.
...
- Persona Pack v1.1.1 does not implement custom characters, persona self-learning, Voice Profile, TTS voice models, Live2D, or LLM-primary state writes. Memory, proactive, and Semantic Shadow write boundaries remain unchanged.
