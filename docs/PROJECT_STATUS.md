...
`reilink-mvp-v0.1.1` 已经作为公开展示版本发布，用于 GitHub / portfolio / interview 展示。`reilink-v0.2-pre` 已作为预发布版本公开，当前 `dev/codex-reilink` 已进一步补齐 standalone runtime / productization foundation，并阶段性完成 Voice Interaction MVP：可选系统 TTS、本地 ASR 主聊天输入、transcript-first 用户确认发送、Local ASR Settings 持久化和 release regression freeze。当前已完成 Overlay v1 Foundation，接入 Voice Interaction v2.1 与 Voice Profile v1，并完成 LLM-primary Guarded Extraction v1 pilot 与 Extraction Eval Runner v0：前景 LLM semantic reader、严格候选 schema、deterministic guard、text / voice_confirmed / voice_direct 来源 metadata、rule fallback、安全 trace 和固定 mock eval 回归。
...
- LLM-primary Guarded Extraction v1 pilot 已完成：`docs/llm_primary_guarded_extraction_architecture.md` 定义并记录当前 LLM-primary foreground semantic reader、schema validation、guard decision、rule fallback、Shadow Mode audit、memory / proactive / persona 边界和 safe trace；配套场景位于 `docs/qa/llm_primary_guarded_extraction_scenarios.json`，固定 eval 场景位于 `docs/qa/extraction_eval_scenarios.json`。
...
- Semantic Extraction v1 pilot 现在有 foreground LLM-primary path：LLM 只提出候选，schema validation 和 deterministic guard 决定 `apply`、`ask_clarification`、`candidate_only`、`no_op` 或 `fallback_to_rule`。只有 `guard_source=llm_primary` 的 deterministic apply path 可以写 game context；memory、proactive 和 persona 不由该路径写入。
...
`reilink-mvp-v0.1.1` has been published as the public showcase version for GitHub, portfolio, and interview presentation. `reilink-v0.2-pre` has been published as a pre-release, and the current `dev/codex-reilink` branch has further filled in standalone runtime / productization foundation while completing a staged Voice Interaction MVP: optional system TTS, main-chat Local ASR input, transcript-first user-confirmed sending, Local ASR Settings persistence, and release regression freeze. Overlay v1 Foundation is complete, Voice Interaction v2.1 plus Voice Profile v1 are wired in, and LLM-primary Guarded Extraction v1.0.1 pilot is implemented with a foreground LLM semantic reader, strict candidate schema, deterministic guard, switch / negation target roles, ASR typo candidates, text / voice_confirmed / voice_direct source metadata, rule grounding / fallback, Direct Conversation partial-transcript guard, and safe trace.
...
- LLM-primary Guarded Extraction v1 pilot: typed text, confirm-send voice transcript, and direct-conversation voice transcript enter the same foreground semantic reader; Pydantic schema validation and deterministic guard only allow low-risk game context fields to apply, while memory / proactive / persona remain out of this write path. v1.0.2 adds compact JSON recovery, compat retry, and ultra-compact fallback. Extraction Eval Runner v0 now covers fixed mock scenarios and an optional live-provider entrypoint.
- Pending Memory confirmation.
...
- LLM-primary Guarded Extraction v1.0.2 pilot is documented in `docs/llm_primary_guarded_extraction_architecture.md`, with machine-readable scenarios in `docs/qa/llm_primary_guarded_extraction_scenarios.json` and eval samples in `docs/qa/extraction_eval_scenarios.json`. It defines the current LLM-primary foreground semantic reader, schema validation, guard decisions, switch / negation role fields, rule grounding / fallback, Shadow Mode audit, memory / proactive / persona boundaries, safe trace, and eval metrics.
...
- Semantic Extraction v1.0.2 pilot now has a foreground LLM-primary path: the LLM only proposes candidates, and tolerant-but-safe schema validation plus deterministic guard decides `apply`, `ask_clarification`, `candidate_only`, `no_op`, or `fallback_to_rule`. The primary parser accepts both the older expanded shape and the minimal `updates` shape, recovers fenced / prefixed / array JSON, retries once without `response_format` when the first primary attempt is invalid JSON or schema invalid, then can retry once more with an ultra-compact key whitelist if compat output is still unstable. Rules act as grounding / safe fallback, not the primary reader. Only deterministic events marked `guard_source=llm_primary` can write game context; memory, proactive, and persona are not written by this path.
