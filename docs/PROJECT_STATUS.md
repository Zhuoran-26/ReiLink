...
- Local ASR transcript-first 默认边界保持稳定：`confirm_send` 下识别结果只填入输入框，不自动发送；用户确认点击发送后才进入 chat flow；未确认 transcript 不进入 memory、prompt、knowledge retrieval 或 game context。`direct_conversation` 是显式 opt-in，转写成功后只通过现有 chat flow 自动发送，不绕过记忆确认或游戏上下文安全门。
...
- Candidate Memory v1 已接入运行时：显式记忆、稳定玩法偏好、稳定互动偏好、剧透偏好和 voice direct 记忆意图会进入 pending candidate；否定记忆、单次 session event、assistant / proactive 来源、secret 和 persona drift 会被 guard 阻断；接受后写入 `long_term_memories` 可见条目并可进入现有 prompt memory context。配套场景位于 `docs/qa/candidate_memory_scenarios.json`。当前仍不实现 Memory Retrieval、Session Archive、向量数据库或外部 memory provider。
...
- Local ASR keeps the transcript-first default boundary: under `confirm_send`, recognized text only fills the input, is not auto-sent, and enters chat flow only after the user confirms by sending. Under `direct_conversation`, transcripts are auto-sent only through the existing chat flow and still do not bypass memory confirmation or game-context safety.
...
- Candidate Memory v1 is now wired into runtime: explicit remember requests, stable gameplay preferences, stable interaction preferences, spoiler preferences, and voice-direct memory intents become pending candidates; negative memory requests, one-off session events, assistant / proactive sources, secrets, and persona drift are blocked by guard; accepted candidates become visible `long_term_memories` and can enter the existing prompt memory context. Machine-readable scenarios live in `docs/qa/candidate_memory_scenarios.json`. Memory Retrieval, Session Archive, vector search, and external memory providers remain unimplemented.
