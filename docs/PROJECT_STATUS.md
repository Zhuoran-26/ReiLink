...
- Session Archive v1 Architecture：新增 `docs/session_archive_v1_architecture.md` 与 30 条 `docs/qa/session_archive_scenarios.json`，定义安全、用户可控的 session 历史摘要层，区分 Session Timeline、Game Session State、Memory Candidate、Long-term Memory 和 PromptMemoryBlock，明确 safe-summary-only、Archive -> Memory Candidate bridge、Search 规划、UI 方向、retention 和 privacy 边界；runtime 尚未实现。
...
- Session Archive v1 Architecture 已新增：`docs/session_archive_v1_architecture.md` 定义 safe session history / search / memory candidate bridge 的产品定位、层级边界、内容边界、默认策略、数据模型草案、Archive -> Memory Candidate 流程、Search 设计、UI 规划和 privacy / safety 规则；配套场景位于 `docs/qa/session_archive_scenarios.json`。当前只完成 architecture / docs / QA，不实现 archive persistence、search runtime、archive UI、prompt archive retrieval 或 vector search。
...
- Session Archive v1 runtime / Archive Search v1 / Archive-to-Memory Candidate v1。
...
- Hermes-style Memory Architecture v0: documents the boundaries between Working Context, Game Session State, Session Timeline, Memory Candidate, Long-term Memory, Retrieved Memory, Prompt Memory Block, Persona Core, and Candidate Game Understanding, with memory architecture QA scenarios. Session Archive v1 Architecture is now added as a docs / QA baseline; Session Archive runtime, vector search, and external memory frameworks remain out of scope.
...
- Session Archive v1 Architecture: adds `docs/session_archive_v1_architecture.md` and 30 `docs/qa/session_archive_scenarios.json` cases, defining a safe, user-controlled session history summary layer, boundaries between Session Timeline, Game Session State, Memory Candidate, Long-term Memory, and PromptMemoryBlock, safe-summary-only content rules, Archive -> Memory Candidate bridge, Search planning, UI direction, retention, and privacy boundaries. Runtime remains unimplemented.
...
- Session Archive v1 Architecture is documented in `docs/session_archive_v1_architecture.md`, with scenarios in `docs/qa/session_archive_scenarios.json`. It defines safe session history / search / memory candidate bridge positioning, layer boundaries, content boundaries, default policy, data-model drafts, Archive -> Memory Candidate flow, Search design, UI planning, and privacy / safety rules. This is architecture / docs / QA only; archive persistence, search runtime, archive UI, prompt archive retrieval, and vector search remain unimplemented.
