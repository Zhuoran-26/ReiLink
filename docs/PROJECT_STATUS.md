...
截至本次阶段冻结，Voice v2.2、Local ASR v1、Local ASR Native File Picker v1 和 Overlay v1.1 macOS safe mode 已作为当前稳定回归基线记录。Voice v2.2 release matrix、hardening checklist 与 release note 草稿已经加入；macOS Overlay auto-show 仍故意 fail-closed，后续恢复必须单独立项并通过 packaged `.app` QA checklist。
...
- Voice Interaction v2.2 + Voice Profile v1 + TTS Strategy v0 + TTS Provider Registry v0：直接语音对话 loop 的九态状态机、`auto_sending` / `interrupted` / `error` 可见反馈、短录音 / 短文本 / 空文本 / 半句 guard、TTS 打断、录音 / 播放互斥、Home / Chat 紧凑状态、Voice workspace Conversation 状态面板、规则化 full / brief / silent 播报策略，以及 renderer 端 `system_speech_synthesis` strategy / fallback 与只读 provider 能力展示；hands-free、wake word、角色 TTS / 角色音色、真实外部 / 本地 TTS provider 和 Overlay voice state 仍未实现。
...
- Voice Interaction v2.2 + Voice Profile v1 + TTS Strategy v0 + TTS Provider Registry v0: nine-state direct conversation loop, visible `auto_sending` / `interrupted` / `error`, short-recording / short-text / empty-text / partial-phrase guards, TTS interruption, recording / speaking mutual exclusion, compact Home / Chat state, Voice workspace Conversation state panel, deterministic full / brief / silent spoken reply policy, renderer-side `system_speech_synthesis` strategy / fallback, and read-only provider capability surface; hands-free, wake word, character TTS / character voice, real external / local TTS providers, and Overlay Voice state remain unimplemented.
...
- Voice Interaction v2.2 + Voice Profile v1 are wired into the renderer. `docs/voice_interaction_v2_spec.md` defines the direct conversation loop, `idle` / `listening` / `transcribing` / `auto_sending` / `ready_to_send` / `assistant_thinking` / `speaking` / `interrupted` / `error` states, confirm-send default, explicit `direct_conversation` opt-in, short-recording / short-text / empty-text / partial-phrase guards, TTS / recording mutual exclusion, Direct Conversation brief auto-speak when Voice Output is enabled, safe error handling, and UI placement. `docs/voice_profile_v1.md` defines full / brief / silent policy, never-spoken categories, and provider event boundaries. The release mapping lives in `docs/qa/voice_v2_2_release_matrix.json`, and the gate checklist lives in `docs/release_voice_v2_2_hardening_checklist.md`. The current implementation scope is not hands-free, wake word, character TTS / character voice, real external / local TTS providers, or Overlay Voice state.
