...
- LLM-first 回复生成。
- 游戏上下文、记忆和知识层提供辅助信息。
- 当前 sample companion 是 ReiLink 原创 Rei-like persona，不使用官方 IP；v1.1 已开始使用中文优先的结构化 Persona Pack 校准冷静寡言陪伴风格。
...
- Prompt Preview。
- Rei Persona Pack v1.1：中文优先的结构化原创 persona 文件、冷静寡言风格校准、safe loader、prompt assembly 接入和 Debug Prompt Preview 安全摘要。
- Debug Dashboard。
...
- Persona Pack v1.1 不做用户自定义角色、Persona 自动学习、Voice Profile、TTS 音色、Live2D 或 LLM-primary 状态写入；memory、proactive 和 Semantic Shadow 的状态写入边界保持不变。
...
- The current sample companion is an original ReiLink Rei-like persona and does not use official IP; v1.1 now uses a Chinese-first structured Persona Pack to calibrate a cold, quiet companion style.
...
- Event Bus / Event Stream.
- Prompt Preview.
- Rei Persona Pack v1.1: Chinese-first structured original persona files, cold quiet companion calibration, safe loader, prompt assembly integration, and safe Debug Prompt Preview summary.
- Debug Dashboard.
- QA / Regression scenarios.
- UI polish.
...
- Persona Pack v1.1 does not implement custom characters, persona self-learning, Voice Profile, TTS voice models, Live2D, or LLM-primary state writes. Memory, proactive, and Semantic Shadow write boundaries remain unchanged.
