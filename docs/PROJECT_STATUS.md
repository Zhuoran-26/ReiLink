...
- Voice Input Capture & Transcript UX Stabilization v0 已接入：主 Local ASR 使用浏览器 `MediaRecorder`，当前没有 Web Audio VAD 或 silence detection；用户主动停止优先，30 秒是明确可见的安全上限。`stop` 后等待最终 `dataavailable` 再生成完整 Blob 和调用 ASR，取消则丢弃本轮音频且不调用 ASR。confirm-send 输出统一标记为可编辑、尚未发送的转写草稿；Direct Conversation 仅在空文本、短文本、短录音、疑似截断 / 半句和明显噪声 guard 全部通过后自动发送。Event Stream 只增加 stop reason、quality、send decision、模式、字数和时长等安全摘要。
...
- Voice Input Capture & Transcript UX Stabilization v0 is wired in. Main Local ASR capture uses browser `MediaRecorder`; there is no Web Audio VAD or silence detection. User stop is primary and 30 seconds is an explicit safety limit. ASR starts only after the final `dataavailable` event has been incorporated into the Blob; cancellation discards the round without ASR. Confirm-send labels output as an editable, unsent transcript draft. Direct Conversation auto-sends only after empty, short-text, short-recording, suspected truncation / partial, and obvious-noise checks pass. Event Stream stores only safe stop-reason, quality, send-decision, mode, count, and duration metadata.
