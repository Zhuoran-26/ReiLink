...
`reilink-mvp-v0.1.1` 已经作为公开展示版本发布，用于 GitHub / portfolio / interview 展示。`reilink-v0.2-pre` 已作为预发布版本公开，当前 `dev/codex-reilink` 已进一步补齐 standalone runtime / productization foundation，并阶段性完成 Voice Interaction MVP：可选系统 TTS、本地 ASR 主聊天输入、transcript-first 用户确认发送、Local ASR Settings 持久化和 release regression freeze。当前已完成 Overlay v1 Foundation，接入 Voice Interaction v2.1 与 Voice Profile v1，并完成 LLM-primary Guarded Extraction v1 pilot 与 Extraction Eval Runner v0 / v1.0.3：前景 LLM semantic reader、严格候选 schema、formal game state 与 candidate understanding 分层、deterministic guard、text / voice_confirmed / voice_direct 来源 metadata、rule fallback、安全 trace、固定 mock eval 回归和可选 live provider drift check。Hermes-style Memory Architecture v0 已作为文档与 QA 场景基线加入，用于后续 Candidate Memory / Memory Retrieval / Session Archive，而不改变当前 runtime。
...
- Hermes-style Memory Architecture v0：文档化 Working Context、Game Session State、Session Timeline、Memory Candidate、Long-term Memory、Retrieved Memory、Prompt Memory Block、Persona Core 与 Candidate Game Understanding 的边界，配套 memory architecture QA scenarios；当前不实现新 memory runtime、向量库或外部 memory framework。
...
- Hermes-style Memory Architecture v0 已新增：`docs/memory_architecture_v0.md` 记录轻量 Hermes research、ReiLink memory 分层、Memory Candidate lifecycle、confirmation flow、prompt assembly、防人格漂移、数据模型草案、与现有模块关系和后续路线；配套场景位于 `docs/qa/memory_architecture_scenarios.json`。当前范围是 docs / QA surface，不实现完整 Memory runtime、Session Archive、Memory Retrieval、Candidate Memory 或外部 memory provider。
...
- Pending Memory confirmation.
- Hermes-style Memory Architecture v0: documents the boundaries between Working Context, Game Session State, Session Timeline, Memory Candidate, Long-term Memory, Retrieved Memory, Prompt Memory Block, Persona Core, and Candidate Game Understanding, with memory architecture QA scenarios. It does not implement a new memory runtime, vector database, or external memory framework.
...
- Hermes-style Memory Architecture v0 is documented in `docs/memory_architecture_v0.md`, with scenarios in `docs/qa/memory_architecture_scenarios.json`. It records lightweight Hermes research, ReiLink memory layers, Memory Candidate lifecycle, confirmation flow, prompt assembly, anti-persona-drift rules, data-model drafts, module relationships, and follow-up roadmap. The current scope is docs / QA surface only.
