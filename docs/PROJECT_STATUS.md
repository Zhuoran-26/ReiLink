...
- LLM-primary Guarded Extraction v1 pilot: typed text, confirm-send voice transcript, and direct-conversation voice transcript enter the same foreground semantic reader; Pydantic schema validation and deterministic guard only allow low-risk game context fields to apply, while memory / proactive / persona remain out of this write path. Eval runner remains unimplemented.
- Pending Memory confirmation.
...
- Voice Interaction v2.1 + Voice Profile v1: direct spoken conversation loop state machine, confirm-send default, explicit opt-in Direct Conversation Mode, TTS interruption, recording / speaking mutual exclusion, compact Home / Chat state, Voice workspace Conversation state panel, and rule-based full / brief / silent spoken reply policy; hands-free, character TTS / character voice, and Overlay voice state remain unimplemented.
...
- LLM-primary Guarded Extraction v1 pilot is documented in `docs/llm_primary_guarded_extraction_architecture.md`, with machine-readable scenarios in `docs/qa/llm_primary_guarded_extraction_scenarios.json`. It defines the current LLM-primary foreground semantic reader, schema validation, guard decisions, rule fallback, Shadow Mode audit, memory / proactive / persona boundaries, and safe trace.
...
- Semantic Extraction v1 pilot now has a foreground LLM-primary path: the LLM only proposes candidates, and schema validation plus deterministic guard decides `apply`, `ask_clarification`, `candidate_only`, `no_op`, or `fallback_to_rule`. Only deterministic events marked `guard_source=llm_primary` can write game context; memory, proactive, and persona are not written by this path.
