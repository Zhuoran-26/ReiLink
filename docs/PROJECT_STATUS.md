...
`reilink-mvp-v0.1.1` 已经作为公开展示版本发布，用于 GitHub / portfolio / interview 展示。`reilink-v0.2-pre` 已作为预发布版本公开，当前 `dev/codex-reilink` 已进一步补齐 standalone runtime / productization foundation，并阶段性完成 Voice Interaction MVP：可选系统 TTS、本地 ASR 主聊天输入、transcript-first 用户确认发送、Local ASR Settings 持久化和 release regression freeze。当前已完成 Overlay v1 Foundation，接入 Voice Interaction v2.1 与 Voice Profile v1，并完成 LLM-primary Guarded Extraction v1 pilot 与 Extraction Eval Runner v0 / v1.0.3：前景 LLM semantic reader、严格候选 schema、formal game state 与 candidate understanding 分层、deterministic guard、text / voice_confirmed / voice_direct 来源 metadata、rule fallback、安全 trace、固定 mock eval 回归和可选 live provider drift check。Hermes-style Memory Architecture v0 已作为文档与 QA 场景基线加入，Candidate Memory v1 已实现最小运行时；Memory Retrieval、Session Archive、向量库和外部 memory provider 仍未实现。
...
- Candidate Memory v1.1 已接入运行时：显式记忆经 Memory Candidate guard 后会自动写入可见 `long_term_memories`，聊天页显示轻量可撤销提示；稳定隐式玩法偏好、稳定互动偏好、剧透偏好等仍进入 pending candidate。Memory Candidate Check 保持 LLM-primary：规则只做 fast prefilter / grounding / deterministic fallback，偏好类输入不会因为没有显式“记住”而被规则层吞掉。否定记忆、单次 session event、assistant / proactive 来源、secret 和 persona drift 会被 guard 阻断；active accepted memory 可进入现有 prompt memory context，undone / inactive、pending、ignored、expired、rejected 不进入 prompt。v1.1.1 对 Rei 回复语气做了小修：memory 状态透明度由聊天页 UI 小字提示承担，Rei 主回复只做自然、克制的 acknowledgement，不暴露 candidate、guard、长期记忆条目或待确认工作流。配套场景位于 `docs/qa/candidate_memory_scenarios.json` 与 `docs/qa/memory_ux_v1_1_scenarios.json`。当前仍不实现 Memory Retrieval、Session Archive、向量数据库或外部 memory provider。
...
- Hermes-style Memory Architecture v0: documents the boundaries between Working Context, Game Session State, Session Timeline, Memory Candidate, Long-term Memory, Retrieved Memory, Prompt Memory Block, Persona Core, and Candidate Game Understanding, with memory architecture QA scenarios. Memory Retrieval, Session Archive, vector search, and external memory frameworks remain out of scope.
