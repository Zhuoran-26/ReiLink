...
- Local ASR v1.1 已补齐输出规范化和 UX polish：`zh-CN` 归一为 whisper `zh`，ASR transcript 返回前 trim / 折叠空白 / 轻量繁转简，默认成功状态提示 `转写完成，请确认后发送`，直接对话模式提示 `转写完成，已自动发送`，timeout 提示可尝试更小模型或更短录音。
...
- Local ASR v1.1 now includes transcript output polish: `zh-CN` is normalized to whisper `zh`, ASR transcripts are trimmed / whitespace-collapsed / lightly normalized to Simplified Chinese before filling the input, default success asks the user to confirm before sending, Direct Conversation success indicates automatic send only after partial-transcript guard passes, and timeouts suggest a smaller model or shorter recording.
