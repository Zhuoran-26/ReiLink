...
`reilink-mvp-v0.1.1` 已经作为公开展示版本发布，用于 GitHub / portfolio / interview 展示。`reilink-v0.2-pre` 已作为预发布版本公开，当前 `dev/codex-reilink` 已进一步补齐 standalone runtime / productization foundation，并阶段性完成 Voice Interaction MVP：可选系统 TTS、本地 ASR 主聊天输入、transcript-first 用户确认发送、Local ASR Settings 持久化和 release regression freeze。当前已完成 Overlay v1 Foundation，接入 Voice Interaction v2.1、Voice Profile v1、TTS Strategy Spike v0、TTS Provider Registry / Capability Surface v0 与 Direct Conversation UX Polish v0，并完成 LLM-primary Guarded Extraction v1 pilot 与 Extraction Eval Runner v0 / v1.0.3：前景 LLM semantic reader、严格候选 schema、formal game state 与 candidate understanding 分层、deterministic guard、text / voice_confirmed / voice_direct 来源 metadata、rule fallback、安全 trace、固定 mock eval 回归和可选 live provider drift check。Hermes-style Memory Architecture v0 已作为文档与 QA 场景基线加入，Candidate Memory v1 已实现最小运行时，Memory Retrieval v1 已接入 accepted / active long-term memory 的 bounded prompt assembly，Persona-Memory Eval v0.1 已加入 mock-first 回归入口与 live warning tiers，Session Archive v1 Architecture / Runtime / Search v1 已加入 safe-summary local archive、最近会话 UI 与安全关键词 / 过滤搜索，Archive-to-Memory Candidate Bridge v0 已加入显式扫描、deterministic safe-summary detector、memory guard / pending queue 复用和用户确认流；Context & Memory release hardening checklist 与 v0.2-pre.4 release notes 草稿已加入。向量库、semantic archive search、prompt archive retrieval、archive search auto-candidate、archive scan audit counts、advanced archive UI、外部 memory provider、真实外部 / 本地 TTS provider、Overlay auto-show 和 Live2D / Vision 仍未实现。
...
- Voice Interaction v2.1 + Voice Profile v1 + TTS Strategy v0 + TTS Provider Registry v0 + Direct Conversation UX Polish v0：直接语音对话 loop 的状态机、confirm-send 默认、直接对话显式 opt-in、`auto_sending` / `interrupted` / `error` 可见反馈、短 / 空 / 半句 transcript guard、TTS 打断、录音 / 播放互斥、Home / Chat 紧凑状态、Voice workspace Conversation 状态面板、规则化 full / brief / silent 播报策略，以及 renderer 端 `system_speech_synthesis` strategy / fallback 与只读 provider 能力展示；hands-free、角色 TTS / 角色音色、真实外部 / 本地 TTS provider 和 Overlay voice state 仍未实现。
...
- TTS Strategy Spike v0 + TTS Provider Registry v0 已接入 renderer：当前唯一启用 / 可选择 provider 是 `system_speech_synthesis`，封装本机系统 `speechSynthesis` 的 speak / stop / availability fallback；Voice workspace 显示当前 provider、状态、能力、隐私边界和 Local TTS / External TTS disabled 占位，Direct Conversation 会把 `brief` / `full` / `silent` profile 传入 TTS lifecycle，Event Stream 只记录 provider / strategy id、provider status、fallback flag、source、profile、字数、状态和错误摘要，不保存完整 assistant reply 或 spoken text。
...
- Voice Interaction v2.1 + Voice Profile v1 + Direct Conversation UX Polish v0 已接入 renderer：`docs/voice_interaction_v2_spec.md` 定义直接语音对话 loop、`idle` / `listening` / `transcribing` / `auto_sending` / `ready_to_send` / `assistant_thinking` / `speaking` / `interrupted` / `error` 状态、confirm-send 默认、`direct_conversation` 显式 opt-in、TTS 与录音互斥、Voice Output 开启时的直接对话默认短版播报、安全错误处理和 UI 放置；短 / 空 / 半句 transcript 不自动发送，并显示可恢复提示。`docs/voice_profile_v1.md` 定义 full / brief / silent 播报策略、never-spoken 类别和 TTS Provider Registry v0 事件边界；配套场景位于 `docs/qa/voice_interaction_v2_scenarios.json` 与 `docs/qa/voice_profile_scenarios.json`。当前实现范围不包含 hands-free、角色 TTS / 角色音色、真实外部 / 本地 TTS provider 或 Overlay voice state。
...
- Voice Output 当前通过 `system_speech_synthesis` strategy 使用系统 `speechSynthesis`，不是角色级配音；“Rei”等名字和语气可能不自然，后续可研究本地角色 TTS 或更自然的 voice provider，但当前不接商业 TTS、不引入外部 API key、不上传音频。
- Voice Input v1 已完成 push-to-talk fallback：`confirm_send` 默认模式下 Web Speech transcript 只填入输入框，不自动发送；未确认 transcript 不进入 memory、prompt、knowledge retrieval 或 game context。v2.1 直接对话模式开启后，Web Speech / Local ASR transcript 会在用户主动录音结束后进入正常 chat flow；空文本、过短文本、过短录音和疑似半句会被拦截为确认发送或重试提示，不写 memory / proactive / game context。
...
- Voice Interaction v2 polish / hands-free and short spoken reply research。
...
- Voice Interaction v2.1 + Voice Profile v1 + TTS Strategy v0 + TTS Provider Registry v0: direct spoken conversation loop state machine, confirm-send default, explicit opt-in Direct Conversation Mode, short / partial transcript auto-send guard, TTS interruption, recording / speaking mutual exclusion, compact Home / Chat state, Voice workspace Conversation state panel, rule-based full / brief / silent spoken reply policy, renderer-side `system_speech_synthesis` strategy / fallback, and read-only provider capability surface; hands-free, character TTS / character voice, real external / local TTS providers, and Overlay voice state remain unimplemented.
...
- TTS Strategy Spike v0 + TTS Provider Registry v0 are wired into the renderer. The only enabled / selectable provider is `system_speech_synthesis`, which wraps local system `speechSynthesis` speak / stop / availability fallback. The Voice workspace shows current provider, status, capabilities, privacy boundary, fallback summary, and Local TTS / External TTS disabled placeholders. Direct Conversation passes `brief` / `full` / `silent` profile metadata into TTS lifecycle events, and Event Stream records only provider / strategy id, provider status, fallback flag, source, profile, character counts, status, and error summaries without storing full assistant replies or spoken text.
...
- Voice Interaction v2.1 + Voice Profile v1 are wired into the renderer. `docs/voice_interaction_v2_spec.md` defines the direct conversation loop, `idle` / `listening` / `transcribing` / `ready_to_send` / `assistant_thinking` / `speaking` / `interrupted` / `error` states, confirm-send default, explicit `direct_conversation` opt-in, short / partial transcript auto-send guard, TTS / recording mutual exclusion, Direct Conversation brief auto-speak when Voice Output is enabled, safe error handling, and UI placement. `docs/voice_profile_v1.md` defines full / brief / silent policy, never-spoken categories, and TTS Provider Registry v0 event boundaries. Machine-readable scenarios live in `docs/qa/voice_interaction_v2_scenarios.json` and `docs/qa/voice_profile_scenarios.json`. The current implementation scope is not hands-free, character TTS / character voice, real external / local TTS providers, or Overlay voice state.
...
- Voice Output currently uses system `speechSynthesis` through the `system_speech_synthesis` strategy, not character-grade voice acting; names like "Rei" and the tone may sound unnatural. A local character TTS or more natural voice provider can be researched later, but commercial TTS, external API keys, and audio upload are not part of the current scope.
- Voice Input v1 push-to-talk fallback is implemented: in the default `confirm_send` mode, Web Speech transcripts only fill the input and are not auto-sent; unconfirmed transcripts do not enter memory, prompt, knowledge retrieval, or game context. In v2.1 Direct Conversation Mode, Web Speech / Local ASR transcripts enter the normal chat flow after the user actively records.
...
- Voice becomes a top-level product module. The current state is Local ASR transcript-first by default plus Voice Output plus Voice Interaction v2.1 Direct Conversation opt-in plus Voice Profile v1 behavior policy plus TTS Strategy v0 system fallback plus TTS Provider Registry v0 read-only capability surface; `docs/voice_interaction_v2_spec.md` covers and now wires `idle`, `listening`, `transcribing`, `ready_to_send`, `assistant_thinking`, `speaking`, `interrupted`, and `error`. Hands-free, character TTS / character voice, real external / local TTS providers, and Overlay voice state remain unimplemented.
