...
- Shadow Mode provider 兼容性回归必须覆盖：首次 `response_format: json_object` 返回 invalid JSON 时，后台 Shadow 可做一次无 `response_format` 的兼容 retry；retry 成功显示 `shadow_succeeded / 兼容模式`，retry 仍失败显示 `shadow_invalid_json / 兼容模式仍 invalid_json`。timeout、auth_failed、provider_unavailable、provider_error 不应 retry。
- Shadow Mode final event 安全诊断必须覆盖：`response_format_used`、`compat_retry_used`、`json_recovery_stage`、`finish_reason`、`content_length_bucket`、`first_char_type`；这些字段不得包含 raw provider response、raw prompt、完整用户输入、路径、API key、`.env`、stdout/stderr。
