...
Audio Capture / Temp File v1 本身仍不会把任何 transcript 填入输入框，不自动发送，不写 memory / prompt，不触发 knowledge retrieval 或 game context extraction；后续 Backend ASR Transcription Bridge 已在下一节按同一安全边界实现。
