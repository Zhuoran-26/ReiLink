...
7. Click `Record & Transcribe`.
   - Expected: transcript fills the chat input.
   - Expected: ReiLink does not auto-send the transcript.
8. Review or edit the transcript manually.
...
| `local_asr_transcription_no_text` | ASR completed but no safe transcript was parsed. | Check microphone input, model language fit, model quality, and output format compatibility. |
| `local_asr_transcription_timed_out` | ASR did not finish before timeout. | Try a smaller model, shorter recording, or faster local hardware. |
...
- Debug Panel does not show raw stdout / stderr, raw exception, or full local paths.
